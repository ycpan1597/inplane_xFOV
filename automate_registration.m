...
start = 60;
step = 1;
last = 80; % -1 uses the length of the stack
...
disp_opt = false;
method = 'mean_squares'; % mean_squares or SURF
...
for i = 1:length(current_file)
    [recon, attr] = panoramic_reconstruct(fullfile(sub_dir, current_file{i}),...
                                          'start_frame', start,...
                                          'frame_step', step,...
                                          'end_frame', last,...
                                          'pad_size', 1,...
    ...
    % imshow(recon, [0, 255]); title(sprintf('Frame %d to %d at %d frame interval\n%d iterations', start, last, step, iters))
