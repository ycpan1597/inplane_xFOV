...
[cAH, baAH, fig, drawAreaPos] = ConfigAxes(fig, params.processCorrelationOnly, params.processBAOnly);

% I added this section
%-----%
if (strcmpi(params.processBAOnly, 'On'))
    stats = struct();
    ...
    title(params.tit, 'FontSize', 16)
    return
elseif strcmpi(params.processCorrelationOnly,'On')
...
while length(varargin)>i
	...
	switch upper(parameter)
        ...
        case 'PROCESSBAONLY', params.processBAOnly = val;
...
function [cAH, baAH, fig, drawAreaPos] = ConfigAxes(fig, processCorrelationOnly, processBAOnly)
...
% pretty sure I added the following section to copy the
% "processCorrelationOnly" above
elseif strcmpi(processBAOnly, 'On')
    cAH = [];
    if isempty(fig)
		fig = figure;
		set(fig,'Units','centimeters','Position',[3 3 10 10],'color','w');
...
h = plot(cAH, l, l, ':', 'linewidth', 2); set(h,'color',[0.6 0.6 0.6],'tag','IdentityLine'); % modified the linewidth of the identity line
...
for groupi=1:params.numGroups
	...
	if strcmpi(params.symbols,'Num')
		...
		ph = plot(baAH,ref,dif,marker,'color',color, 'linewidth', 3);
		set(ph,'markersize',params.markerSize);
	end
end
axis(baAH,'square')
xlabel(baAH,params.meanLabel); ylabel(baAH,params.deltaLabel);
params = FormatPlotAxes(baAH, data, params); % update plot axes; otherwise params.axesLimits = 'auto' and error occurs
...
% 	end
	...
	text(0,0,params.tit,'HorizontalAlignment','Center','VerticalAlignment','Middle','FontSize',20,'FontWeight','bold','Interpreter','Tex','Parent',titleAH)
	set(fig,'Units',units);
end
