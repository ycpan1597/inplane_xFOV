...
function [previous_image, attr] = panoramic_reconstruct(file_path, varargin)
    ...
    p.addParameter('rm_offset', false);
    ...
    % add some preprocessing steps here
    BW = imbinarize(mydata_bw/255, 0.05);
    for i = 1:size(mydata_bw, 3)
        mydata_bw(:, :, i) = mydata_bw(:, :, i) .* bwareaopen(BW(:, :, i), 10000);
        if nnz(mydata_bw(:, :, i)) == 0
            error('At least one frame (at %d) has nothing');
        end
    end
    ...
    frames_to_read = start_frame:frame_step:end_frame;
    ...
    overscan_RAs = imref2d(overscan_size * (size(ref) - [2*pad_size, 2*pad_size]), [over_scan_xMinWorldLimit, over_scan_xMaxWorldLimit], [over_scan_yMinWorldLimit, over_scan_yMaxWorldLimit]);
    % keeps the overall image size the same, whether there is padding or
    % not
...
%     RegularStepGradientDescent parameters
    ...
    tic
    for i = 1:length(frames_to_read) - 1
        
        current = data_to_register(:, :, frames_to_read(i));
        next = data_to_register(:, :, frames_to_read(i+1));
...
%                  error('Bad registration at frame %d', frames_to_read(i));
%              end
        end
